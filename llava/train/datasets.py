...
from mm_utils import expand2square
...
class INFormatDataset(Dataset):
    ...
    def __init__(self, 
        ...
        self.data_args = data_args
        self.safe_loading = safe_loading
        self.max_num_images = getattr(self.data_args, "max_num_images", 10)
        self.clip_processor = self.data_args.image_processor
        self.image_aspect_ratio = self.data_args.image_aspect_ratio
        self.max_tokens = tokenizer.model_max_length
        ...
        if not os.path.isdir(data_path):
            ...
            if data_path.endswith(".json"):
                ...
                with open(data_path, "r") as f:
                    ...
                    self.data = [item for item in jsonl_data if 'content_image' in item and len(item['content_image']) > 0]
            else:
    ...
    def load_image(self, image_path_or_url):
        image_path_or_url = os.path.join('/'.join(self.image_path.split("/")[:-1]), image_path_or_url)
    ...
    def parse_sample(self, sample):
        # images = sample["content_image"]

        texts = sample["md"]
        pattern = r'<figure><img src="(.*?)">'
        # 提取所有匹配的图片路径
        images = re.findall(pattern, texts)
        ...
        #    ]
        #)
        valid_image = [True] * len(images)
        #for _img, _imgmeta in zip(images, metadata):
        #    assert( _img is None) == (_imgmeta is None), sample
        return images, texts, valid_image
    ...
    def getitem(self, index):
        ...
        # parse sample and check
        images, text, valid_image = self.parse_sample(sample)
        ...
        # preprocess and pad images
        images_tensors = preprocess_image(images, self.clip_processor, self.image_aspect_ratio)
        # if (len(images) < self.max_num_images) and len(images) != 0:
            # print(index,len(images_tensors))
        zero_padding = torch.zeros(
            (
                self.max_num_images - len(images_tensors),
                N_CHANNELS,
        ...
        # print(len(images_tensors),len(zero_padding))
        images_tensors = torch.cat((images_tensors, zero_padding), dim=0)
        ...
        text = re.sub(r'<figure><img src=".*?">', '<image>', text, count=len(images))
