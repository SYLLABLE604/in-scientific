...
import sys
sys.path.append(os.path.abspath("/root/model/multimodal_textbook/llava"))
sys.path.append(os.path.abspath("/root/model/multimodal_textbook/llava/train"))

from constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN
from llava_trainer import LLaVAInterLeavedTrainer, SetSharedEpochCallback
from model.language_model.llava_llama import LlavaLlamaForCausalLM
import conversation as conversation_lib
import conversation_2 as conversation_lib_2
from model import *
...
class DataArguments:
    ...
    image_aspect_ratio: Optional[str] = field(default='square')
    data_type: str = field(default="mmc4")
    max_num_images: int = field(default=12)
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    output_dir: Optional[str] = field(default="facebook/opt-125m")
...
class LazySupervisedDataset(Dataset):
    ...
    def __getitem__(self, i) -> Dict[str, torch.Tensor]:
        ...
        if 'content_image' in sources[0]:
            if len(sources[0]["content_image"]) > 0:
                processor = self.data_args.image_processor
                image = self.load_image(self.list_data_dict[i]['content_image'])
...
def make_in_format_data_module(tokenizer, data_args):
    # for our textbook interleaved:  dataset and collator
    assert len(data_args.data_path) == 1, data_args.data_path
    assert len(data_args.image_folder) == 1, data_args.image_folder
    data_args.data_path = data_args.data_path[0]
    ...
    #from pretrain_llava.llava.train.datasets import ObelicsFormatDataset
    from datasets import INFormatDataset
    train_dataset = INFormatDataset(data_path, img_path, tokenizer, data_args)
...
def make_supervised_data_module(tokenizer: transformers.PreTrainedTokenizer,
    ...
    elif "inscientific" in data_args.data_type.lower():
        return make_in_format_data_module(tokenizer, data_args)
...
def check_pretrained_load(model, model_args):
    ...
    for key in model_state_dict.keys():
        if key in loaded_state_dict:
            ...
            # print(f"Key: {key}, loaded_p dtype: {loaded_p.dtype}, model_p dtype: {model_p.dtype}, loaded_p device: {loaded_p.device}, model_p device: {model_p.device}")
...
def train(attn_implementation=None):  
    ...
    model.config.max_num_images = data_args.max_num_images
    ...
    if (
        ...
        or ("obelisc" in data_args.data_type.lower()) or ("inscientific" in data_args.data_type.lower())
    ):
        # import ipdb; ipdb.set_trace()
